...
Mesmo usando threads, se várias delas pegarem portas filtradas, o scanner inteiro congela. A solução aplicada aqui foi transformar os sockets em Não-Bloqueantes *(O_NONBLOCK)*, disparar a conexão instantaneamente e usar o `select()` para controlar um timeout agressivo e customizado de apenas 1.5 segundos.
...
* **Asynchronous Multiplexing:** Utilização do `select()` monitorando estritamente os descritores de escrita *(write_fds)* para determinar quando o Three-Way Handshake terminou em background.
...
* **Gatilhos do select() na Escrita:** Aprendi que para capturar o fim de um `connect()` não-bloqueante, o `select()` precisa monitorar a escrita, pois o socket se torna gravável tanto quando a conexão é estabelecida com sucesso quanto quando ela é sumariamente rejeitada (recebimento de *RST*).
